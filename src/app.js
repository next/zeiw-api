...
db.exec('create table if not exists users (id integer primary key not null, name text not null, avatar text, access_token text not null, refresh_token text not null, flags integer not null, faction integer)')

const registerStatement = db.prepare('insert into users (id, name, avatar, access_token, refresh_token, flags, faction) values(?, ?, ?, ?, ?, ?, ?)')
  ...
  try {
    ...
    if (splitUrl[0] === '/v1/login' && req.method === 'GET') {
      ...
      if (req.method === 'GET') {
        ...
        const factionResult = editFactionStatement.run(factions[body.faction], sqlite.Integer(tokenData.id))
